...
const AddPhilosophy = () => {
  ...
  const createTourist = async (title, description) => {
    try {
      setLoading(true);
      ...
      const response = await API.post("/philosophy", formData);
  ...
  //     const response = await API.post("/philosophy/category", formData);
